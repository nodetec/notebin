...
const ConnectedRelaysStatus = () => {
  ...
  const relayConnectionStateColors = isLoading
    ? { bg: "bg-yellow-500 dark:bg-yellow-400", fg: "text-yellow-500 dark:text-yellow-400" }
    : error
    ...
    <button
      ...
      <span>
        {isLoading ? "Connecting..." :
          connectedRelays ? `Connected to ${connectedRelaysCount} relay${connectedRelaysCount > 1 ? "s" : ""} 📡` : "Not connected"}
      </span>
      ...
      {(connectedRelaysCount > 0 ) ? (
        <span className={`absolute z-50 hidden group-focus:flex mx-auto min-w-max -bottom-4 right-0 items-center gap-4 flex-col translate-y-full border border-current p-4 rounded-md bg-zinc-300 dark:bg-neutral-800 ${relayConnectionStateColors.fg}`}>
