...
const Popup = ({ title, isOpen, setIsOpen, children }: PopupProps) => {
  ...
  return (
    <div className="z-50 fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/4 border-2 border-neutral-500 rounded-md">
      <Button
      ...
      <div className="bg-primary flex flex-col justify-center items-stretch gap-4 p-6 shadow-overlay">
        <h3 className="text-xl text-neutral-400 text-center pb-4">{title}</h3>
        {children}
      </div>
    </div>
  );
